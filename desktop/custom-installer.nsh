!macro customInit
  nsExec::ExecToLog "taskkill /f /im OpenSpace.exe"
  Sleep 1000
!macroend

!macro customCheckAppRunning
  nsExec::Exec '"$SYSDIR\cmd.exe" /C tasklist /FI "IMAGENAME eq OpenSpace.exe" | "$SYSDIR\find.exe" "OpenSpace.exe"'
  Pop $R0
  ...
  ${if} $R0 == 0
    ...
    doStopProcess:
      ...
      nsExec::ExecToLog '"$SYSDIR\taskkill.exe" /F /IM "OpenSpace.exe"'
      Sleep 1000
...
!macro customUnInstall
  ...
  deleteData:
    RMDir /r "$APPDATA\openspace-desktop"
  skipDelete:
!macroend
