...
test.describe("Settings account tab (paid mode)", () => {
  ...
  test("Connection toggle shows OpenSpace Account as active", async () => {
    ...
    await expect(toggle.getByText("OpenSpace Account")).toBeVisible();
  ...
  test("balance section is visible in paid mode", async () => {
    ...
    const hasSubscribe = await page
      .getByText("OpenSpace Subscription")
      .isVisible()
      .catch(() => false);
