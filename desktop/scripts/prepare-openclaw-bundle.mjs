...
const repoRoot = path.resolve(appRoot, "..", "openclaw");
...
// Rewrite `import fileType from "file-type/core.js"` →
// `import * as fileType from "file-type"` so it works with file-type v21+
// which has no default export and only exposes subpath "." in exports map.
function createFileTypeRewritePlugin() {
  return {
    name: "file-type-rewrite",
    setup(build) {
      build.onResolve({ filter: /^file-type\/core\.js$/ }, (args) => {
        return { path: args.path, namespace: "file-type-shim" };
      });
      build.onLoad({ filter: /.*/, namespace: "file-type-shim" }, () => {
        return {
          contents: `
            import { fileTypeFromBuffer, fileTypeFromStream } from "file-type";
            export default { fromBuffer: fileTypeFromBuffer, fromStream: fileTypeFromStream };
            export { fileTypeFromBuffer, fileTypeFromStream };
          `,
          loader: "js",
        };
      });
...
async function main() {
  ...
  if (fs.existsSync(entryJs)) {
    ...
    // (rollup configs, test dirs with dev-only imports like leakage/ioredis)
    {
      ...
      // separately on Windows; on macOS they are loaded at runtime from the deployed store.
      const extDir = path.join(vendorDistDir, "extensions");
      if (fs.existsSync(extDir)) {
        let cleaned = 0;
        for (const ext of fs.readdirSync(extDir, { withFileTypes: true })) {
          if (!ext.isDirectory()) continue;
          const nm = path.join(extDir, ext.name, "node_modules");
          if (fs.existsSync(nm)) {
            fs.rmSync(nm, { recursive: true, force: true });
            cleaned++;
          }
        }
        console.log(`[electron-desktop] Removed node_modules from ${cleaned} extensions`);
      }
    }
    ...
    // metafile disabled due to esbuild JSON serialization crash on large bundles.
    // External packages are derived from the effective externals list instead.
    for (const ext of effectiveExternals) {
      const name = ext.replace(/\/\*$/, "");
      if (!NODE_BUILTINS.has(name) && !name.startsWith("node:")) {
        externalPkgs.add(name);
      }
    }
