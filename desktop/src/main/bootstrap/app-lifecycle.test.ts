...
describe("registerProtocolHandler", () => {
  ...
  it("registers protocol for packaged app", () => {
    ...
    registerProtocolHandler("openspace");
    expect(app.setAsDefaultProtocolClient).toHaveBeenCalledWith("openspace");
  });
...
describe("registerAppLifecycle", () => {
  ...
  it("registers handlers and runs before-quit flow", async () => {
    ...
    expect(ok).toBe(true);
    handlers.get("second-instance")?.({}, ["openspace://foo"]);
