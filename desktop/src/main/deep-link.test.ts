...
describe("parseDeepLinkUrl", () => {
  it("parses a valid deep link URL", () => {
    const result = parseDeepLinkUrl("openspace://callback?code=abc&state=xyz");
  ...
  it("parses URL with pathname", () => {
    const result = parseDeepLinkUrl("openspace://auth/callback?token=123");
    expect(result).toEqual({
      host: "auth",
      pathname: "/callback",
      params: { token: "123" },
  ...
  it("parses URL with no query params", () => {
    const result = parseDeepLinkUrl("openspace://open");
...
describe("handleDeepLink", () => {
  it("sends parsed deep link data to the window", () => {
    ...
    handleDeepLink("openspace://callback?code=abc&state=xyz", win);
  ...
  it("does nothing when window is null", () => {
    expect(() => handleDeepLink("openspace://callback?code=abc", null)).not.toThrow();
  });

  it("does nothing when window is destroyed", () => {
    const win = makeFakeWindow(true);
    handleDeepLink("openspace://callback?code=abc", win);
